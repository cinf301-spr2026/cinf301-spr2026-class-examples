...
function makePerson(first, last) {
  return {
    ...
    fullNameReversed: () => this.last + ", " + this.first,
  };
}
...
function Person(first, last) {
  ...
  this.fullNameReversed = () => this.last + ", " + this.first
};
...
// A better way using arrays! Much simpler - just use 'this' directly
String.prototype.reversed = function () {
  return this.split("").reverse().join("");
};

// Alternative using spread operator (ES6+)
String.prototype.reversedSpread = function () {
  return [...this].reverse().join("");
};

// Alternative using Array.from()
String.prototype.reversedArrayFrom = function () {
